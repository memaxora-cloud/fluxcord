...
async function loginModal(mode = 'login') {
  ...
  openModal(`
    <div class="auth-switch">
      <button class="auth-tab ${!isRegister ? 'active' : ''}" id="loginTab">Login</button>
      <button class="auth-tab ${isRegister ? 'active' : ''}" id="registerTab">Register</button>
    </div>
    ${isRegister ? `
      <h2>Create your account</h2>
      <p class="muted">Verify your email once, then use your password for future logins.</p>
      <form class="form" id="registerRequestForm">
        <label>Name *<input id="regName" maxlength="60" autocomplete="name" required></label>
        <label>Email *<input id="regEmail" type="email" autocomplete="email" required></label>
        <p class="form-note">Please check your inbox as well as spam folder.</p>
        <label>Password *<input id="regPassword" type="password" minlength="6" autocomplete="new-password" required></label>
        <label>Number<input id="regPhone" inputmode="tel" autocomplete="tel" placeholder="01XXXXXXXXX"></label>
        <button class="primary-button" type="submit">Send OTP →</button>
        <button class="secondary-button" type="button" id="cancelAuth">← Cancel</button>
      </form>
    ` : `
      <h2>Welcome back</h2>
      <p class="muted">Login with your email and password.</p>
  ...
  if (isRegister) {
    $('#registerRequestForm').addEventListener('submit', async (event) => {
      ...
      try {
        ...
        const result = await api('/api/auth/request-otp', { method: 'POST', body: JSON.stringify({ email }) });
        if (!result.sent && !result.demoOtp) {
          throw new Error('OTP email could not be sent. Please try again later.');
        }
        openOtpVerify(email, result.demoOtp, { name: $('#regName').value.trim(), password: $('#regPassword').value, phone: $('#regPhone').value.trim() });
...
async function continueAfterAuth() {
  if (!state.currentUser?.name?.trim()) await requiredNameModal();
  const pending = localStorage.getItem('fluxcord_pending_checkout');
  if (pending) { localStorage.removeItem('fluxcord_pending_checkout'); await goToCheckout(pending); }
...
function checkoutId() {
  const match = window.location.pathname.match(/^\/checkout\/([^/]+)\/?$/i);
...
async function goToCheckout(existingId = '') {
  if (!state.currentUser) {
    ...
    localStorage.setItem('fluxcord_pending_checkout', id);
    await loginModal('login');
    toast('Please login or register to continue checkout.');
    return;
  }
  if (!state.cart.length) { toast('Your cart is empty.'); return; }
  const id = existingId || checkoutId() || generateCheckoutId();
  window.history.pushState({}, '', `/checkout/${encodeURIComponent(id)}`);
...
function selectedPaymentAmount(total, method) {
  return method === 'bKash' || method === 'Nagad' ? money(total) : `৳${Number(total || 0).toLocaleString('en-BD')} value`;
}
...
async function renderCheckoutPortal() {
  ...
  const portal = $('#checkoutPortal'); portal.classList.remove('hidden');
  const products = state.cart.map(item => ({ ...item, product: state.products.find(p => p.id === item.id) })).filter(x => x.product);
  const total = cartTotal();
  const paymentOptions = Object.keys(state.paymentMethods).map(method => `<option value="${escapeHtml(method)}">${escapeHtml(method)}</option>`).join('');
  portal.innerHTML = `
    <div class="checkout-shell container">
      <div class="checkout-top"><button class="secondary-button" id="checkoutBack">← Back</button><span class="checkout-brand">⚡ FluxCord Checkout</span><span class="checkout-id">#${escapeHtml(checkoutId())}</span></div>
      <div class="checkout-layout">
        <div class="checkout-main-card">
          <div class="checkout-heading"><span class="section-label">SECURE CHECKOUT</span><h1>New Payment Request <span>🔥</span></h1><p>Complete your details, choose a payment method and submit the transaction ID.</p></div>
          <form class="form checkout-form" id="checkoutPortalForm">
            <label>Name *<input value="${escapeHtml(state.currentUser.name || '')}" readonly></label>
            <label>Email *<input value="${escapeHtml(state.currentUser.email || '')}" readonly></label>
            <label>Social media or Phone Number *<span class="field-help">Please choose a contact method in case we need to contact you.</span>
              <div class="contact-grid"><select id="contactType"><option value="Discord">Discord</option><option value="Instagram">Instagram</option><option value="Facebook">Facebook</option><option value="Phone">Phone</option></select><input id="contactValue" placeholder="Your username or number" required></div>
            </label>
            <label>Payment Method *<select id="paymentMethod">${paymentOptions}</select></label>
            <div class="payment-request-box" id="paymentRequestBox"></div>
            <label>Transaction ID *<input id="trxid" maxlength="120" placeholder="Paste your transaction ID" required></label>
            <div id="bdtNumberWrap" class="hidden"><label>bKash/Nagad Number *<input id="senderNumber" inputmode="tel" placeholder="01XXXXXXXXX"></label></div>
            <label>Coupon Code <input id="couponCode" placeholder="Optional"></label>
            <div id="checkoutCoupon" class="info-box">Total: <strong>${money(total)}</strong></div>
            <div class="checkout-actions"><button class="secondary-button" type="button" id="checkoutBackBottom">← Back</button><button class="primary-button" type="submit">Submit →</button></div>
          </form>
        </div>
        <aside class="checkout-summary"><span class="section-label">YOUR PRODUCTS</span><div class="checkout-products">${products.map(x => `<div class="checkout-product"><span>${escapeHtml(x.product.name)} <small>× ${x.qty}</small></span><strong>${money(x.product.price_bdt*x.qty)}</strong></div>`).join('')}</div><div class="checkout-total"><span>Total</span><strong>${money(total)}</strong></div><p>After verification, your product will appear here and be delivered to <strong>${escapeHtml(state.currentUser.email)}</strong>.</p></aside>
      </div>
    </div>`;

  const updatePayment = () => {
    const method = $('#paymentMethod').value; const address = state.paymentMethods[method];
    $('#paymentRequestBox').innerHTML = `<div><strong>Send ${selectedPaymentAmount(total, method)}</strong><small>${escapeHtml(method)} destination</small></div><code>${escapeHtml(address || '')}</code>`;
    $('#bdtNumberWrap').classList.toggle('hidden', !(method === 'bKash' || method === 'Nagad'));
    $('#senderNumber').required = method === 'bKash' || method === 'Nagad';
  };
  $('#paymentMethod').addEventListener('change', updatePayment); updatePayment();
  $('#checkoutBack').onclick = closeCheckout; $('#checkoutBackBottom').onclick = closeCheckout;
  $('#couponCode').addEventListener('blur', async () => { const code=$('#couponCode').value.trim(); if(!code)return; try { const r=await api('/api/coupon/check',{method:'POST',body:JSON.stringify({code})}); const d=r.coupon.type==='percent'?Math.floor(total*r.coupon.value/100):Math.min(total,r.coupon.value); $('#checkoutCoupon').innerHTML=`Coupon applied: <strong>${escapeHtml(r.coupon.code)}</strong> · Final total: <strong>${money(Math.max(0,total-d))}</strong>`; } catch(e){ $('#checkoutCoupon').textContent=e.message; } });
  $('#checkoutPortalForm').addEventListener('submit', async event => {
    event.preventDefault(); const submit=event.submitter; submit.disabled=true; submit.innerHTML='<span class="button-loader"></span> Processing...';
    try { const result=await api('/api/orders',{method:'POST',body:JSON.stringify({items:state.cart,email:state.currentUser.email,phone:$('#contactValue').value.trim(),contact_type:$('#contactType').value,contact_value:$('#contactValue').value.trim(),payment_method:$('#paymentMethod').value,trxid:$('#trxid').value.trim(),sender_number:$('#senderNumber').value.trim(),coupon:$('#couponCode').value.trim()})}); state.cart=[]; saveCart(); renderCheckoutSuccess(result); } catch(error) { toast(error.message); submit.disabled=false; submit.innerHTML='Submit →'; }
  });
}
...
async function renderReviews() {
  try {
    ...
    const rows = [0, 1].map((rowIndex) => {
      const rowReviews = source.filter((_, index) => index % 2 === rowIndex);
...
init().then(async () => { if (checkoutId() && state.currentUser) await renderCheckoutPortal(); else if (checkoutId()) await goToCheckout(checkoutId()); });

window.addEventListener('popstate', () => { if (checkoutId()) renderCheckoutPortal(); else closeCheckout(); });
