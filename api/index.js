...
app.get('/api/health', async (req, res) => {
  ...
  return res.json({ ok: true, service: 'fluxcord', database });
});
...
app.post('/api/auth/request-otp', async (req, res) => {
  ...
  const { data: existingOtp } = await supabase.from('otps').select('created_at').eq('email', email).maybeSingle();
  if (existingOtp && Date.now() - new Date(existingOtp.created_at).getTime() < 60000) {
    const wait = Math.ceil((60000 - (Date.now() - new Date(existingOtp.created_at).getTime())) / 1000);
    return fail(res, 429, `Please wait ${wait}s before requesting another OTP.`);
  }
  ...
  if (!sent && !DEMO_OTP) {
    return fail(res, 503, 'OTP email could not be sent. Check the SMTP settings in Vercel Environment Variables.');
  }
...
app.post('/api/auth/verify-otp', async (req, res) => {
  ...
  const requestedName = String(req.body.name || '').trim().replace(/\s+/g, ' ').slice(0, 60);
  ...
  if (!user) {
    const result = await supabase
      ...
      .insert({
        ...
        password_hash: requestedPassword.length >= 6 ? hashPassword(requestedPassword) : null
      })
      .select('*')
      .single();
    ...
    await supabase
      .from('users')
      .update({ verified: true, ...(requestedName ? { name: requestedName } : {}), ...(requestedPhone ? { phone: requestedPhone } : {}), ...(requestedPassword.length >= 6 ? { password_hash: hashPassword(requestedPassword) } : {}) })
      .eq('id', user.id);
    if (requestedName) user.name = requestedName;
    if (requestedPhone) user.phone = requestedPhone;
  }
...
app.post('/api/auth/login', async (req, res) => {
  ...
  if (!validEmail(email) || !password) return fail(res, 400, 'Email and password are required.');
  const { data: user, error } = await supabase.from('users').select('*').eq('email', email).maybeSingle();
  if (error) return fail(res, 500, 'Could not login right now.');
  if (!user) return fail(res, 401, 'Account not found. Please register first.');
  if (!user.password_hash) return fail(res, 401, 'This account was created with email OTP. Please register again to set a password.');
  if (!verifyPassword(password, user.password_hash)) return fail(res, 401, 'Incorrect email or password.');
  setSession(res, { id: user.id, email: user.email, name: user.name || '', admin: false });
  return res.json({ ok: true, user: { id: user.id, email: user.email, name: user.name || '', phone: user.phone || '' } });
});
...
app.get('/api/admin/script', auth, adminOnly, (req, res) => {
  try {
    const scriptPath = path.join(__dirname, '..', 'private', 'admin.js');
...
app.post('/api/orders', auth, async (req, res) => {
  ...
  const email = cleanEmail(req.body.email);
  const phone = String(req.body.phone || req.body.contact_value || '').trim().slice(0, 40);
  const contactType = String(req.body.contact_type || 'Phone').trim().slice(0, 20);
  const contactValue = String(req.body.contact_value || phone).trim().slice(0, 80);
...
app.get('/api/tickets/:id/messages', auth, async (req, res) => {
  ...
  const { data: ticket } = await supabase
    .from('tickets')
    .select('id,email,user_id,order_id,status')
    .eq('id', ticketId)
    .maybeSingle();
  ...
  if (!req.user.admin && ticket.user_id !== req.user.id && ticket.email !== req.user.email) {
